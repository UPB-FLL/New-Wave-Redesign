import type { DivisionServiceContent } from '../types';

export const integration: DivisionServiceContent = {
  slug: 'integration',
  navLabel: 'Integration',
  metaTitle: 'Booking & CRM integration, South FL | New Wave: Social Engineering',
  metaDescription:
    'Booking and CRM integration for South Florida small businesses: connect your website, scheduling, forms, email, SMS, and analytics. Book a discovery call.',
  keywords:
    'booking and crm integration, marketing technology integration for small business, crm integration fort lauderdale, booking system integration south florida, website and crm integration broward county, email and sms integration palm beach, conversion tracking setup miami-dade, lead routing for small business',
  serviceType: 'Marketing technology integration',
  icon: 'integration',
  kicker: 'Integration',
  headline: 'Booking and marketing integration that connects what already works',
  summary:
    'New Wave: Social Engineering connects the tools South Florida businesses already use, including the website, booking and scheduling system, forms, CRM, email and SMS lists, listings, and analytics, into one path from discovery to booking. The goal isn’t necessarily to replace the systems already working, it’s to connect them through a clearer customer experience. Being part of New Wave IT helps here, since connecting systems cleanly and testing them before anyone relies on them is everyday IT work.',
  cardSummary:
    'Connect your website, booking tools, forms, CRM, and analytics into one path from discovery to booking, without replacing what already works.',
  scope: [
    {
      title: 'One booking path from every channel',
      detail:
        'Whether someone finds you through a post, an ad, a search, or a listing, the “Book” button leads to the booking, registration, or scheduling tool you already use. No one has to start over or call to finish.',
    },
    {
      title: 'Forms to CRM, email, and SMS lists',
      detail:
        'Inquiry, signup, and booking forms send contacts into your CRM and the right email or SMS lists, with consent captured and fields that match. No more copying names from an inbox into a spreadsheet.',
    },
    {
      title: 'Lead routing',
      detail:
        'Every inquiry, whether it comes from a website form, a social message, a listing, or an email, goes to a named person with a clear next step. Where your tools allow it, an automatic reply tells people when to expect an answer, so inquiries don’t sit unanswered.',
    },
    {
      title: 'Analytics and conversion tracking',
      detail:
        'We set up analytics and conversion tracking across the whole path, from the post, ad, or listing to the finished booking, and test that each step records correctly. You can see which sources lead to bookings and where people drop off.',
    },
    {
      title: 'Phone and email inquiries',
      detail:
        'Calls and emails are part of the path too. The phone numbers and email addresses on your website, listings, and ads reach the right person, and those inquiries are counted alongside forms and online bookings.',
    },
  ],
  process: [
    {
      title: 'Discover',
      detail:
        'We map the tools you use today: the website, booking and registration portals, forms, CRM, email and SMS, listings, and social accounts. Then we follow the path a customer takes from first search to confirmed booking and note where it breaks or goes quiet.',
    },
    {
      title: 'Prioritize',
      detail:
        'We rank the gaps by what they cost the business, such as missed inquiries, double entry, or bookings you can’t trace to a source, and agree on what to connect first. Where a tool is working, we keep it.',
    },
    {
      title: 'Connect and test',
      detail:
        'We build the connections, set up routing and tracking, and test every path end to end with test inquiries and bookings before anything goes live.',
    },
    {
      title: 'Document and review',
      detail:
        'You get a plain-language map of how the pieces connect and who owns each account. After launch, we review the data and fix anything that isn’t recording or routing correctly.',
    },
  ],
  deliverables: [
    'Map of your current tools and customer path',
    'Website-to-booking connections, tested end to end',
    'Forms connected to your CRM and email and SMS lists',
    'Lead routing rules and automatic replies',
    'Analytics and conversion tracking across the path',
    'Documentation of every connection and account owner',
  ],
  metrics: [
    'Bookings traced to their source',
    'Inquiry response time',
    'Inquiries reaching the right person',
    'Drop-off points between tools',
    'Contacts captured without manual entry',
  ],
  faqs: [
    {
      question: 'What does booking and CRM integration include?',
      answer:
        'It connects your website, booking or scheduling tools, forms, CRM, email and SMS lists, listings, and analytics so information moves between them without retyping. It also covers lead routing, so every inquiry reaches a person, and conversion tracking, so you can see which sources lead to bookings. In most cases we keep the tools you already use, and if one can’t do what the business needs, we explain the options before anything changes.',
    },
    {
      question: 'Where does an integration project start?',
      answer:
        'The first step is a discovery call about the tools you use and where inquiries get lost today. From there we trace a real booking from first search to confirmation and agree on which connections to fix first. How long the work takes depends on how many tools are involved and what access already exists, which becomes clear during that review.',
    },
    {
      question: 'How do you measure whether the integration is working?',
      answer:
        'Every connection is tested before launch with test inquiries and bookings, and we check the data afterward to confirm contacts, bookings, and conversions are recorded correctly. From there, we measure against bookings and inquiries traced to their source, how quickly new inquiries get a response, and where people drop off. While we work together, these go into a monthly report.',
    },
    {
      question: 'Who owns the accounts, and will you coordinate with our IT provider?',
      answer:
        'Your business owns its accounts, data, and tools, and the connections we build run under your accounts, not ours. We can work alongside your current web host, IT provider, agency, or in-house team, and coordinating with other technical teams is routine for us. You also get plain-language documentation of how everything connects, so nothing depends on one person’s memory.',
    },
  ],
};
